...
my_dogs.execute("""DROP TABLE IF EXISTS Dogs""")
...
my_dogs.execute("""INSERT INTO Dogs (DogName, OwnerName, Breed)
                VALUES ('Fido', 'Bob', 'Lab'), ('Rover', 'Sally', 'Poodle'), ('Spot', 'Tom', 'Dalmation'),
                ('Lady', 'Sarah', 'German Shepard')
                """)
...
my_dogs.execute("""SELECT * FROM Dogs""")
...
# (remove the # to test)
my_dogs.execute("""SELECT * FROM Dogs WHERE DogID == 2""")
...
my_dogs.execute("""UPDATE Dogs SET OwnerName = "Unknown" WHERE DogID == 2""")
print(my_dogs.fetchone())
